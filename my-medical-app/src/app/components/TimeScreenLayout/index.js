...
const Index = () => {
  ...
  const renderLocation = (item, index) => (
    <Grid item xs={12} md={6} key={index}>
      <Card variant="outlined" sx={{ px: 4, borderRadius: "15px", pt: 3, pb: 3,  }}>
            ...
            <Grid container spacing={1} sx={{ backgroundColor: "#FBF9F8",}}>
              {item.buttons.map((button, buttonIndex) => (
                <Grid item xs={6} sm={4} key={buttonIndex}>
                  ...
                  >
                    <Link href="/TimeFixtureLayout" style={{ cursor: "pointer", textDecoration: "none", listStyle: "none", color:"#284473" }} passHref>
                      <Typography style={{ cursor: "pointer", textDecoration: "none", listStyle: "none", fontFamily: "Livvic-Medium" }}>
                        {button.time}
                      </Typography>
                    </Link>
                  </Box>
  ...
  return (
    <Card variant="outlined" sx={{ px: 2, borderRadius: "15px", pt: 5, pb: 3, marginTop: "16px" }}>
      <Box display="flex" alignItems="center" paddingBottom="20px" position="relative">
        ...
        />
        {showDatePicker && (
          <Box sx={{ position: 'absolute', top: '40px', right: '0', zIndex: 1000 }}>
            <DatePicker
